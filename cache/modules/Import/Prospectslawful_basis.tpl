...
{else}
	...
	{assign var="field_options" value=$fields.lawful_basis.options }
	{capture name="field_val"}{$fields.lawful_basis.value}{/capture}
	{assign var="field_val" value=$smarty.capture.field_val}
	{capture name="ac_key"}{$fields.lawful_basis.name}{/capture}
	{assign var="ac_key" value=$smarty.capture.ac_key}

			<input type="hidden" id="{$fields.lawful_basis.name}_multiselect"
		name="{$fields.lawful_basis.name}_multiselect" value="true">
		{multienum_to_array string=$fields.lawful_basis.value default=$fields.lawful_basis.default assign="values"}
		<select style='display:none' id="{$fields.lawful_basis.name}"
		name="{$fields.lawful_basis.name}[]"
		multiple="true" size='6' style="width:150" title='' tabindex="1"  
		        >
		{html_options options=$fields.lawful_basis.options selected=$values}
		...
		<input
	    id="{$fields.lawful_basis.name}-input"
	    name="{$fields.lawful_basis.name}-input"
	    size="60"
	    type="text" style="vertical-align: top;">
	
	<span class="id-ff multiple">
	    <button type="button">
	    	<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.lawful_basis.name}-image">
	    	</button>
	    	<button type="button"
	        id="btn-clear-{$fields.lawful_basis.name}-input"
	        title="Clear"
	        onclick="SUGAR.clearRelateField(this.form, '{$fields.lawful_basis.name}-input', '{$fields.lawful_basis.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><span class="suitepicon suitepicon-action-clear"></span></button>
	</span>
		...
		YUI().use('datasource', 'datasource-jsonschema', function (Y) {
					SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.ds = new Y.DataSource.Function({
					    source: function (request) {
					    	var selectElem = document.getElementById("{/literal}{$fields.lawful_basis.name}{literal}");
					    	var ret = [];
	...
	YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
	    ...
	    SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.lawful_basis.name}-input');
	    SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.lawful_basis.name}-image');
	    SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.lawful_basis.name}');
		...
		if({/literal}SUGAR.AutoComplete.{$ac_key}.minQLen{literal} == 0){
		    ...
		    function updateTextOnInterval(){
		    	if (document.activeElement != document.getElementById("{/literal}{$fields.lawful_basis.name}-input{literal}"))
