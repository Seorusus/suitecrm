...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_TYPE_CALL_LIST' module='Calls'}{/capture}
{$label|strip_semicolon}:
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.type_call_list_c.name}" 
id="{$fields.type_call_list_c.name}" 
title=''       
>
{if isset($fields.type_call_list_c.value) && $fields.type_call_list_c.value != ''}
{html_options options=$fields.type_call_list_c.options selected=$fields.type_call_list_c.value}
{else}
{html_options options=$fields.type_call_list_c.options selected=$fields.type_call_list_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.type_call_list_c.options }
{capture name="field_val"}{$fields.type_call_list_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.type_call_list_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.type_call_list_c.name}" 
id="{$fields.type_call_list_c.name}" 
title=''          
>
{if isset($fields.type_call_list_c.value) && $fields.type_call_list_c.value != ''}
...
<input
id="{$fields.type_call_list_c.name}-input"
name="{$fields.type_call_list_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.type_call_list_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.type_call_list_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.type_call_list_c.name}-input', '{$fields.type_call_list_c.name}');sync_{$fields.type_call_list_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.type_call_list_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.type_call_list_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.type_call_list_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.type_call_list_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.type_call_list_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.type_call_list_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.type_call_list_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_CALLS_CUSTO_CONTACT_1_FROM_CUSTO_CONTACT_TITLE' module='Calls'}{/capture}
{$label|strip_semicolon}:
...
value="{$fields.calls_custo_contact_1custo_contact_idb.value}">
<span class="id-ff multiple">
...
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"calls_custo_contact_1custo_contact_idb","name":"calls_custo_contact_1_name"}}{/literal}, 
"single", 
true
...
addToValidate('EditView', 'custo_company_calls_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CUSTO_COMPANY_CALLS_FROM_CUSTO_COMPANY_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'incom_incoming_marketing_calls_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_INCOM_INCOMING_MARKETING_CALLS_1_FROM_INCOM_INCOMING_MARKETING_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'calls_custo_contact_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CALLS_CUSTO_CONTACT_1_FROM_CUSTO_CONTACT_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'custo_user_calls_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CUSTO_USER_CALLS_1_FROM_CUSTO_USER_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'leads_calls_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_LEADS_CALLS_1_FROM_LEADS_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'asterisk_user_ext_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ASTERISK_USER_EXT' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'asterisk_inbound_ext_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ASTERISK_INBOUND_EXT' module='Calls' for_js=true}{literal}' );
...
addToValidate('EditView', 'type_call_list_c', 'enum', false,'{/literal}{sugar_translate label='LBL_TYPE_CALL_LIST' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'asterisk_call_id_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ASTERISK_CALL_ID' module='Calls' for_js=true}{literal}' );
